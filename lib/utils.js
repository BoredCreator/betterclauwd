...
// Max image size the Anthropic API accepts (decoded bytes).
// We target 4.5 MB to leave headroom for base64 overhead and the rest of the request.
const MAX_IMAGE_BYTES = 4.5 * 1024 * 1024

// Convert file to base64, automatically compressing if the image exceeds MAX_IMAGE_BYTES.
// Uses Canvas to resize / reduce JPEG quality so large phone photos are accepted without error.
export const fileToBase64Compressed = async (file) => {
  // Fast path: small file, no compression needed
  if (file.size <= MAX_IMAGE_BYTES) {
    return fileToBase64(file)
  }

  // Load image into a canvas element for compression
  ...
  return new Promise((resolve) => {
    ...
    const attempt = () => {
      ...
      canvas.toBlob((blob) => {
        ...
        if (blob.size <= MAX_IMAGE_BYTES || (quality <= 0.4 && scale <= 0.3)) {
          // Good enough — convert blob to base64
          const reader = new FileReader()
          reader.onload = () => {
            resolve({
          ...
          // Still too big — reduce quality first, then scale
          if (quality > 0.5) {
            quality = Math.max(quality - 0.15, 0.4)
          } else {
...
// Anthropic accepts PDFs up to 32MB. We cap a little under that.
const MAX_PDF_BYTES = 30 * 1024 * 1024

export const fileToPdfDocument = async (file) => {
  if (file.size > MAX_PDF_BYTES) {
    throw new Error(`PDF too large (${Math.round(file.size / 1024 / 1024)}MB). Max 30MB.`)
  }
  const { base64, mimeType, name } = await fileToBase64(file)
  return { base64, mimeType: mimeType || 'application/pdf', name, kind: 'pdf' }
}
...
export const getImageFromClipboard = async (clipboardData) => {
  ...
  for (let i = 0; i < items.length; i++) {
    if (items[i].type.indexOf('image') !== -1) {
      ...
      return fileToBase64Compressed(blob)
    }
  }
  return null
}
