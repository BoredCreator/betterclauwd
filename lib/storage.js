// Incognito (session-only) state — held in module memory, not localStorage.
// When true, getApiKeys reads sessionKeys instead of localStorage, and chat
// persistence is skipped. Everything dies when the tab closes.
let incognito = false
let sessionKeys = {}

export const setIncognito = (on) => {
  incognito = !!on
  if (!on) sessionKeys = {}
}

export const isIncognito = () => incognito
...
export const getApiKeys = () => {
  ...
  // Incognito: keys live only in module memory, never on disk
  if (incognito) return { ...sessionKeys }
  try {
...
export const setApiKey = (provider, key) => {
  if (typeof window === 'undefined') return
  if (incognito) {
    if (key) sessionKeys[provider] = key
    else delete sessionKeys[provider]
    return
  }
...
export const saveChat = async (chat) => {
  if (typeof window === 'undefined') return
  // Incognito: never write conversations to disk
  if (incognito) {
    return {
      ...chat,
...
const DEFAULT_SETTINGS = {
  ...
  mathEnabled: false, // Enable math/computation tools
...
// Price per 1M tokens (input/output) - updated March 2026
